...
def main():
    ...
    db=mysql.connector.connect("localhost","milanoff","Password1","Sbex")
    cursor=db.cursor()
    z=db.cursor()
    ...
    football = soup.find_all('div', class_='event-holder holder-complete')
    for game in football:
        ...
        z.execute("""INSERT into results(acAdi,SiteID,FTHome,FTDraw,FT10,FT12,FT02,Ust,Alt,Tarih,LigAdi) values(%s,%s,%s,%s,%s)""",(team[1],team[0],points[1],points[0],gameID[0]))
        db.commit()
    db.close()
